...
jest.mock('../../database/db', () => {
  return {
    prepare: jest.fn((query) => {
      ...
      if (q.includes('insert into activitylog')) {
        return { run: mockInsertLogRun }
      }
      if (q.includes('insert into affectedrecords')) {
        return { run: mockInsertAffectedRun }
      }
      return { run: jest.fn() }
    }),
...
describe('Logging Service', () => {
  ...
  test('catches errors gracefully and returns false if the database crashes', async () => {
    mockInsertLogRun.mockImplementationOnce(() => {
      throw new Error('Disk full! SQLite crash simulated!')
    })
